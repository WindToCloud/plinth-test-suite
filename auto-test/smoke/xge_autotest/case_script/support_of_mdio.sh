...
function ge_read_phy_register()
{
    ...
    mii-tool ${local_tp1} -v > ${BaseDir}/log/read_phy_register.txt
    if [ $? -eq 0 ];then
        MESSAGE="PASS"
	echo ${MESSAGE}
    else
...
function ge_iperf_read_phy_register()
{
    ...
    iperf -c ${remote_tp1_ip} -t 3600 -i 1 -P 3 > ${BaseDir}/log/ge_iperf_read_phy_register.txt &
    sleep 3
    mii-tool ${local_tp1} -r
    #sleep 1
    #bandwidth1=$(cat ${BaseDir}/log/ge_iperf_read_phy_register.txt | tail -1 | awk '{print $(NF-1)}')
    #echo $bandwidth1
    #if [ x"${bandwidth1}" = x"0" ];then
    #    enableok=1
    #fi
    sleep 20
    #when set sleep within 5s,the data within txt is no include 0.00 ,so wait 20 to let it done
    cat ${BaseDir}/log/ge_iperf_read_phy_register.txt
    bandwidth1=`cat ${BaseDir}/log/ge_iperf_read_phy_register.txt | grep 0.00 | wc -l`
    echo "The count of bandwidth of 0.00 is "${bandwidth1}
    enableok=`awk -v a=$bandwidth1 -v b=0 'BEGIN{print(a>b)?"1":"0"}'`

    echo "Check enableok "${enableok}
    sleep 10
    bandwidth2=$(cat ${BaseDir}/log/ge_iperf_read_phy_register.txt | tail -1 | awk '{print $(NF-1)}')
    echo $bandwidth2
...
function xge_read_phy_register()
{
    ...
    #mii-tool ${remote_fibre1} -v | grep "failed: Invalid argument" > ${BaseDir}/log/xge_read_phy_register.txt

    #mii-tool can not carry out correct ,so no return can be used by grep
    mii-tool ${remote_fibre1} -v

    #if [ $? -eq 0 ];then
    #xge is not support auto negotiation ,so when error occur ,test is success
    if [ $? -eq 1 ];then
        MESSAGE="PASS"
	echo ${MESSAGE}
    else
