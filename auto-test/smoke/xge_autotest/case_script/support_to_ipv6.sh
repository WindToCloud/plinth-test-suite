...
function ge_ipv6_ping_pack()
{
    ...
    ping6 ${RemoteIpv6Address}%${local_tp1} -c 5 > ${BaseDir}/log/ipv6_ping_pack.txt &
    ping6 ${RemoteIpv6Address}%${local_tp1} -c 5
    sleep 10
    echo  `cat ${BaseDir}/log/ipv6_ping_pack.txt`
    cat ${BaseDir}/log/ipv6_ping_pack.txt | grep "received, 0% packet loss"
    if [ x"$?" = x"0" ];then
        MESSAGE="PASS"
        echo ${MESSAGE}
    else
...
function ge_set_ipv6_address()
{
    ...
    ssh root@${BACK_IP} "ping6 ${local_tp1_ipv6_ip}%${remote_tp1} -c 5" > ${BaseDir}/log/set_ipv6_address.txt &
...
function xge_ipv6_ping_pack()
{
    ...
    ping6 ${RemoteIpv6Address}%${local_fibre1} -c 5 > ${BaseDir}/log/ipv6_ping_pack.txt &
    sleep 10
    cat ${BaseDir}/log/ipv6_ping_pack.txt | grep "received, 0% packet loss" >/dev/null
    if [ $? -eq 0 ];then
        MESSAGE="PASS"
...
function xge_set_ipv6_address()
{
    ...
    ssh root@${BACK_IP} "ping6 ${local_fibre1_ipv6_ip}%${remote_fibre1} -c 5" > ${BaseDir}/log/set_ipv6_address.txt &
    cat ${BaseDir}/log/ipv6_ping_pack.txt | grep "received, 0% packet loss" >/dev/null
    if [ $? -eq 0 ];then
