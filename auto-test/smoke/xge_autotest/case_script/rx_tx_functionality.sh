...
function ge_two_ends_receiving_and_sending ()
{
    ...
    ssh root@$BACK_IP 'ping '${local_tp1_ip}' -c 5' > ${BaseDir}/log/ge_two_ends_receiving_and_sending.txt &
    sleep 10
    cat ${BaseDir}/log/ge_two_ends_receiving_and_sending.txt | grep "received, 0% packet loss" >/dev/null
    if [ $? -eq 0 ];then
       disableok=1
...
function xge_two_ends_receiving_and_sending ()
{
    ...
    ping ${remote_fibre2_ip} -c 5 > ${BaseDir}/log/xge_two_ends_receiving_and_sending.txt &
    sleep 10
    ...
    ssh root@$BACK_IP 'ping '${local_fibre2_ip}' -c 5' > ${BaseDir}/log/xge_two_ends_receiving_and_sending.txt &
    sleep 10
    cat ${BaseDir}/log/xge_two_ends_receiving_and_sending.txt | grep "received, 0% packet loss" >/dev/null
    if [ $? -eq 0 ];then
       disableok=1
